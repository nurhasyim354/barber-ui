...
import {
  buildAppointmentSlotOptions,
  addCalendarDaysToDayKey,
} from '@/lib/bookingAppointmentSlots';
import {
...
interface TenantInfo {
  ...
  /** Kunci hari ini `YYYY-MM-DD` (zona kuota server). */
  quotaTodayDayKey?: string;
  /** true = boleh pilih tanggal booking maju. */
...
export function BookingFlow({ variant = 'customer', bottomNav }: BookingFlowProps) {
  ...
  /** Tanggal kunjungan `YYYY-MM-DD` (kalender zona kuota; default dari outlet). */
  const [bookingDayKey, setBookingDayKey] = useState('');
  const [selectedAppointmentSlot, setSelectedAppointmentSlot] = useState<{
    start: string;
    end: string;
  } | null>(null);
  const [staffSelfQueueRow, setStaffSelfQueueRow] = useState<StaffQueueRow | null>(null);
  const [staffSelfQueueLoading, setStaffSelfQueueLoading] = useState(false);
  ...
  const bookingTenantIdRef = useRef<string | undefined>(undefined);
  ...
  useEffect(() => {
    const k = tenant?.quotaTodayDayKey;
    const tid = tenant?._id;
    if (!k || !/^\d{4}-\d{2}-\d{2}$/.test(k)) return;
    if (bookingTenantIdRef.current !== tid) {
      bookingTenantIdRef.current = tid;
      setBookingDayKey(k);
    }
  }, [tenant?.quotaTodayDayKey, tenant?._id]);

  useEffect(() => {
    setSelectedAppointmentSlot(null);
    setStaffSelfQueueRow(null);
  }, [bookingDayKey, selectedStaff?.staffId, isStaffVariant, user?.staffId]);

  useEffect(() => {
    if (bookStep !== 'staff' || !effectiveTenantId || !bookingDayKey) return;
    if (!/^\d{4}-\d{2}-\d{2}$/.test(bookingDayKey)) return;
    let cancelled = false;
    setStaffQueueLoading(true);
    void api
      .get(
        `/tenants/${effectiveTenantId}/staff/queue?date=${encodeURIComponent(bookingDayKey)}`,
      )
      .then((r) => {
        if (cancelled) return;
        if (Array.isArray(r.data)) {
          setStaffQueue(r.data);
          setSelectedStaff((prev) => {
            if (!prev) return null;
            const next = (r.data as StaffQueueRow[]).find((x) => x.staffId === prev.staffId);
            return next ?? null;
          });
        } else {
          setStaffQueue([]);
      ...
      .finally(() => {
        if (!cancelled) setStaffQueueLoading(false);
      });
    return () => {
      cancelled = true;
    };
  }, [bookStep, effectiveTenantId, bookingDayKey]);
  ...
  useEffect(() => {
    ...
    if (guestBookingFlow) {
      const qd =
        bookingDayKey && /^\d{4}-\d{2}-\d{2}$/.test(bookingDayKey)
          ? `?date=${encodeURIComponent(bookingDayKey)}`
          : '';
      const id = setInterval(() => {
        void api
          ...
          .catch(() => {});
        api
          .get(`/tenants/${effectiveTenantId}/staff/queue${qd}`)
          .then((r) => {
    ...
    return () => clearInterval(id);
  }, [user, effectiveTenantId, bookStep, loadBookingData, isStaffVariant, guestBookingFlow, bookingDayKey]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api
        .get(`/tenants/${tenantIdParam}`)
        .then((t) => {
          const dk = t.data?.quotaTodayDayKey;
          const qd =
            dk && /^\d{4}-\d{2}-\d{2}$/.test(String(dk))
              ? `?date=${encodeURIComponent(String(dk))}`
              : '';
          return api.get(`/tenants/${tenantIdParam}/staff/queue${qd}`).catch(() => ({
            data: [] as StaffQueueRow[],
          }));
        }),
    ])
      .then(([svcRes, qRes]) => {
  ...
  const minBookDate = tenant?.quotaTodayDayKey ?? '';
  const maxBookDate =
    tenant?.quotaTodayDayKey && tenant.allowBookOnFutureDates
      ? addCalendarDaysToDayKey(tenant.quotaTodayDayKey, 60)
      : tenant?.quotaTodayDayKey ?? '';

  const slotContextRow = useMemo((): StaffQueueRow | null => {
    if (isStaffVariant && user?.staffId) return staffSelfQueueRow;
    return selectedStaff;
  }, [isStaffVariant, user?.staffId, staffSelfQueueRow, selectedStaff]);

  const needsAppointmentSlot = isStaffVariant || !!selectedStaff;

  const appointmentSlotOptions = useMemo(
    () => (needsAppointmentSlot ? buildAppointmentSlotOptions(slotContextRow) : []),
    [needsAppointmentSlot, slotContextRow],
  );

  useEffect(() => {
    if (!dialogOpen || !needsAppointmentSlot) return;
    if (appointmentSlotOptions.length !== 1) return;
    const o = appointmentSlotOptions[0];
    setSelectedAppointmentSlot({ start: o.start, end: o.end });
  }, [dialogOpen, needsAppointmentSlot, appointmentSlotOptions]);

  useEffect(() => {
    if (!dialogOpen) {
      setStaffSelfQueueRow(null);
      setStaffSelfQueueLoading(false);
      return;
    }
    if (!isStaffVariant || !user?.staffId || !bookingDayKey) {
      setStaffSelfQueueLoading(false);
      return;
    }
    if (!/^\d{4}-\d{2}-\d{2}$/.test(bookingDayKey)) return;
    let c = false;
    setStaffSelfQueueLoading(true);
    void api
      .get(`/staff/${user.staffId}/queue?date=${encodeURIComponent(bookingDayKey)}`)
      .then((r) => {
        if (!c && r.data) setStaffSelfQueueRow(r.data as StaffQueueRow);
      })
      .catch(() => {
        if (!c) setStaffSelfQueueRow(null);
      })
      .finally(() => {
        if (!c) setStaffSelfQueueLoading(false);
      });
    return () => {
      c = true;
    };
  }, [dialogOpen, isStaffVariant, user?.staffId, bookingDayKey]);
  ...
  useEffect(() => {
    ...
    void api
      .get<{ occupiedSeatPositions?: number[] }>(
        `/public/tenants/${effectiveTenantId}/occupied-seat-positions${qSeat}`,
      )
      .then((res) => {
        if (cancelled) return;
  ...
  }, [dialogOpen, seatSlotCount, effectiveTenantId, bookingDayKey]);
  ...
  const handleBook = async () => {
    ...
    const slotNeeded = isStaffVariant || !!selectedStaff;
    if (slotNeeded && isStaffVariant && staffSelfQueueLoading) {
      toast.error('Tunggu sebentar, memuat jadwal slot.');
      return;
    }
    if (slotNeeded) {
      if (appointmentSlotOptions.length === 0) {
        toast.error('Tidak ada jadwal slot untuk staff pada tanggal ini.');
        return;
      }
      if (!selectedAppointmentSlot) {
        toast.error('Pilih jam kunjungan.');
        return;
      }
    }
    setSubmitting(true);
    try {
      const res = guestBookingFlow && tenantIdParam
        ? await api.post(`/public/tenants/${tenantIdParam}/bookings`, {
            ...
            notes,
            ...(bookingDayKey ? { bookingDate: bookingDayKey } : {}),
            ...(slotNeeded && selectedAppointmentSlot ? { appointmentSlot: selectedAppointmentSlot } : {}),
        ...
        >
                ...
                {activeBookings.map((ab, idx) => (
                  <Box key={ab._id} sx={idx > 0 ? { mt: 2.5, pt: 2.5, borderTop: 1, borderColor: 'divider' } : {}}>
                    <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 1.25, flexWrap: 'wrap' }}>
                      ...
                      {ab.appointmentSlot?.start && ab.appointmentSlot?.end && (
                        <Chip
                          icon={<AccessTimeIcon sx={{ fontSize: '14px !important' }} />}
                          label={formatAppointmentSlotLabel(ab.appointmentSlot)}
                          size="small"
                          variant="outlined"
                          color="info"
                          sx={{ fontWeight: 700 }}
          ...
          {bookStep === 'service' && (
            <>
              ...
              {isStaffVariant && tenant?.quotaTodayDayKey && (
                <Box sx={{ mb: 3 }}>
                  {tenant.allowBookOnFutureDates ? (
                    <TextField
                      type="date"
                      label="Tanggal kunjungan"
                      size="small"
                      value={bookingDayKey}
                      onChange={(e) => setBookingDayKey(e.target.value)}
                      InputLabelProps={{ shrink: true }}
                      inputProps={{ min: minBookDate, max: maxBookDate }}
                      fullWidth
                      sx={{ maxWidth: 320 }}
                    />
                  ) : (
                    <Typography variant="body2" color="text.secondary">
                      ...
                      <strong>
                        {bookingDayKey
                          ? new Date(`${bookingDayKey}T12:00:00`).toLocaleDateString('id-ID', {
                              weekday: 'long',
                              day: 'numeric',
                              month: 'long',
                              year: 'numeric',
              ...
              {staffQueueLoading ? (
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {filteredStaffQueue.map((b) => {
                    ...
                    const staffDayClosed = b.canBookOnSelectedDay === false;
                    const cardDisabled =
                      !!tenant?.subscriptionOverdue ||
                      outletQuotaFull ||
                      tenantSlotsExceededForCart ||
      ...
      >
        ...
        <DialogContent sx={{ pt: 0 }}>
          ...
          >
            ...
            {needsAppointmentSlot && (
              <Box sx={{ mt: 1.5, pt: 1.5, borderTop: 1, borderColor: 'divider' }}>
                <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 1 }}>
                  Pilih jam kunjungan
                </Typography>
                {isStaffVariant && staffSelfQueueLoading ? (
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, py: 1 }}>
                    <CircularProgress size={20} />
                    <Typography variant="body2" color="text.secondary">Memuat jadwal…</Typography>
                  </Box>
                ) : appointmentSlotOptions.length === 0 ? (
                  ...
                  <RadioGroup
                    value={
                      selectedAppointmentSlot
                        ? `${selectedAppointmentSlot.start}|${selectedAppointmentSlot.end}`
                        : ''
                    }
                    onChange={(e) => {
                      const v = e.target.value;
                      const [start, end] = v.split('|');
                      setSelectedAppointmentSlot({ start, end });
                    }}
                  >
                    {appointmentSlotOptions.map((o) => (
                      <FormControlLabel
                        key={`${o.start}-${o.end}`}
                        value={`${o.start}|${o.end}`}
                        control={<Radio size="small" />}
                        label={<Typography variant="body2">{formatAppointmentSlotLabel(o)}</Typography>}
                      />
                    ))}
                  </RadioGroup>
                )}
